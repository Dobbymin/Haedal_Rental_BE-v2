 ...
 *
 */
export const findOneUser1 = (req: Request, res: Response) => {
  const user_id = req.query.user_id;

  //filter라는 함수는 자바스크립트에서 배열 함수이다. 필터링을 할때 많이 사용된다 필터링한 데이터를 새로운 배열로 반환한다.
  const user = users.filter((data) => data.id === Number(user_id));
 ...
 */
export const findOneUser2 = (req: Request, res: Response) => {
 ...
 */
export const setUsers = (req: Request, res: Response) => {
  // 구조분해를 통해 id 와 name을 추출
  const { id, name } = req.body;
...
export const setUser = (req: Request, res: Response) => {
  ...
  //map 함수는 자바스크립트에서 배열 함수이다. 요소를 일괄적으로 변경할 때 사용됩니다.
  const user = users.map((data) => {
    if (data.id === Number(user_id)) data.name = name;
